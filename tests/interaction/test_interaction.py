...
class InteractionTest(TestCase):
    ...
    def setUp(self):
        _, self.schnet_activations, __ = test_utils.initialize_and_predict_schnet(r_cutoff=self.r_cutoff, sort_nl_indices=True)
    ...
    def initialize_schnax(self):
        ...
        schnet_inputs, __, ___ = test_utils.initialize_and_predict_schnet(sort_nl_indices=True)
        ...
        R, Z, box, neighbors, displacement_fn = test_utils.initialize_schnax()
        object.__setattr__(neighbors, 'idx', nl)

        layer_outputs, pred = test_utils.predict_schnax(R, Z, displacement_fn, neighbors, self.r_cutoff)
    ...
    def test_distance_expansion(self):
        ...
        schnax_edR = self.schnax_activations['distance_expansion']
