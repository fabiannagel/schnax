...
from tests import test_utils, activation_utils
...
class InteractionTest(TestCase):
    ...
    def setUp(self):
        _, self.schnet_activations, __ = test_utils.initialize_and_predict_schnet(geometry_file=self.geometry_file, weights_file=self.weights_file, r_cutoff=self.r_cutoff, sort_nl_indices=True)
    ...
    def initialize_schnax(self):
        ...
        schnet_inputs, __, ___ = test_utils.initialize_and_predict_schnet(geometry_file=self.geometry_file, weights_file=self.weights_file, r_cutoff=self.r_cutoff, sort_nl_indices=True)
        ...
        R, Z, box, neighbors, displacement_fn = test_utils.initialize_schnax(geometry_file=self.geometry_file, r_cutoff=self.r_cutoff)
        object.__setattr__(neighbors, 'idx', nl)

        layer_outputs, pred = test_utils.predict_schnax(R, Z, displacement_fn, neighbors, self.r_cutoff, weights_file=self.weights_file)
    ...
    def test_distance_expansion(self):
        ...
        schnax_edR = test_utils.get_distance_expansion(self.schnax_activations)
