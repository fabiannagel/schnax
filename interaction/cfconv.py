...
class CFConv(hk.Module):
    ...
    def __init__(self, n_in: int, n_filters: int, n_out: int, filter_network, cutoff_network, activation, normalize_filter=False, axis=2):
        super().__init__(name="CFConv")

        self.in2f = hk.Linear(n_filters, with_bias=False)
        self.f2out = hk.Sequential([hk.Linear(n_out, with_bias=True), activation])
    ...
    def __call__(self, x: jnp.ndarray, dR: jnp.ndarray, neighbors: NeighborList, pairwise_mask: jnp.ndarray, dR_expanded: jnp.ndarray):
